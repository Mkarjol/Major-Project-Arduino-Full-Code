...
float voltage, current, pf, energy, frequency, Act_Power, R_Power,Apa_Power ;
/*///++++++++////*/
const int trigPin = 5;
...
#define WIFI_SSID "Ani"
#define WIFI_PASSWORD "kumara*123"
...
void loop(){
  ...
  R_Power = (voltage*current*(sqrt((pf^2)-1)));
  //ADD REACTIVE ENERGY IF POSSIBLE
  /*///++++++/////*/
...
//  Serial.println("%");
  ...
  /*^^^^^^^^^^^^^^^^*/
    ...
    if (Firebase.RTDB.setFloat(&fbdo, "Main/Electrical/REAL_POWER", R_Power)){
       Serial.println("PASSED");
