...
void setup(){
  ...
  */ //%%%%%%%%%%%%%%%%%%%%%% uncomment this line wile calibrating(end)
  ...
  MQ8.setR0(0.18);  //this will assign valus to R0, comment this 4 lines while calibrating
  MQ6.setR0(0.27);
  MQ7.setR0(0.26);
  MQ4.setR0(0.30);
...
void loop(){
  ...
  /////////////// 'himidity in RH' to 'moisture in ppm' convertion code, used curve fitting to get relation equation 
  float x111 = 1+pow((t/23772.42),2.456301);
  float x11 = (539495600+(34.38525-539495591)/x111);
  float x1 = h*0.01*x11;
  float x2 = (h*20/43)*0.01*x11;  //edit here for moisture calibration
  ...
  if(moisture>=60||Oil_level<=50||t>=75){
      Alart1=1;
      Alart_message1 = "Emergency Non-Electrical fault occured: Transformer turned off";
      digitalWrite(CB_Pin1, true);
      digitalWrite(buzzer1, HIGH);
  }
  else if(moisture>=35||H2>=100||C2H2>=100||CO>=60||CH4>=60||Oil_level<=60||t>=70){ //this line will check for faulty conditions and report fault and reason
    Alart1=1;
