...
int AA1 = 1; //veriables used for teligram notification
int AA2 = 0;
int BB1 = 1;
int BB2 = 0;
float voltage, current, pf, energy, frequency, Act_Power, Rec_Power,Apa_Power,Act,Apa,Rec;
float voltage2=0,current2=0;
const int CB_Pin = 14; //variable used to operate CB by authority,from web&app
bool CB_status = false;
const int CB_PinB = 27; //variable used to operate CB during electrical and non elec emeargency, without controle of authority
...
void setup() {
  ...
  ////////////
  bot.sendMessage(chat_id, "IoT Transformer Bot started up", ""); //teligram notification about start of processor
  ////////
  /*
  ...
  */
  ...
  MQ8.setR0(0.60);  //this will assign valus to R0, comment this 4 lines while calibrating
  MQ135.setR0(2.99);
  MQ7.setR0(1.31);
  MQ4.setR0(0.59);
...
void loop() {
  /////now mpk started editing//////
  /////////////
  voltage = pzem.voltage();
  if(voltage>=0 && voltage<=15000){
    ...
    voltage = 0,current = 0,frequency = 0,energy =0,pf = 0,Act = 0,Rec = 0,Apa = 0;
  }
  //////////////non ele//////
  ...
  if ((voltage>=5) && (frequency>=5) && (voltage>=250||voltage<=210||current>=4.348||current==0.00||pf<=0.3||frequency>=51.5||frequency<=48.5)){
    Alart2=1;
    LedE = 1;
    if(current==0.00){
      Alart_message2 = "Transformer is turned OFF";
      if(AA1==1){
      bot.sendMessage(chat_id, "Transformer is turned OFF");
      }
      AA1 = 0;
      AA2 = 1;
    }
    ...
    else if(pf!=0.00 && pf<=0.3){
      Alart_message2 = " --Low pf--pf is lesser then 0.3";
      if(AA1==1){
      bot.sendMessage(chat_id, "--Low pf--pf is lesser then 0.3");
      }
      AA1 = 0;
      AA2 = 1;
    }
    else if(frequency<=48.50){
      Alart_message2 = "--Under frequency-- frequency is lower then 48.5Hz";
      if(AA1==1){
      bot.sendMessage(chat_id, "--Under frequency-- frequency is lower then 48.5Hz");
      }
      AA1 = 0;
      AA2 = 1;
    }
    else if(frequency>=51.5){
      Alart_message2 = "--Over frequency-- frequency is higher then 51.5";
      if(AA1==1){
      bot.sendMessage(chat_id, "--Over frequency-- frequency is higher then 51.5");
      }
      AA1 = 0;
      AA2 = 1;
    }
  ...
  else if((moisture>=35||H2>=100||CO2>=100||CO>=60||CH4>=60)||(Oil_level<=60 && Oil_level>=10)||(t>=65)){
    Alart1=1;
    LedN = 1;
    if(t>=65){
      Alart_message1 = "Oil temperature is greater then 65*C";
      if(BB1==1){
        bot.sendMessage(chat_id, "Non_Electrical fault occured: Oil temperature is greater then 65*C");
      }
      BB1 = 0;
      BB2 = 1;
    }
      ...
      if(BB1==1){
        bot.sendMessage(chat_id, "Non_Electrical fault occured: OIL level is lesser then 60%");
      }
      BB1 = 0;
      BB2 = 1;
    }
      ...
      if(BB1==1){
        bot.sendMessage(chat_id, "Non_Electrical fault occured: Moisture is higher then 35ppm");
      }
      BB1 = 0;
      BB2 = 1;
    }
      ...
      if(BB1==1){
        bot.sendMessage(chat_id, "Non_Electrical fault occured: H2 gas level is higher then 100ppm");
      }
      BB1 = 0;
      BB2 = 1;
    }
    else if(CO2>=100){
      Alart_message1 = "CO2 gas level is higher then 100ppm";
      if(BB1==1){
        bot.sendMessage(chat_id, "Non_Electrical fault occured: CO2 gas level is higher then 100ppm");
      }
      BB1 = 0;
      BB2 = 1;
    }
    else if(CO>=60){
      Alart_message1 = "CO gas level is higher then 60ppm";
      if(BB1==1){
        bot.sendMessage(chat_id, "Non_Electrical fault occured: CO gas level is higher then 60ppm");
      }
      BB1 = 0;
      BB2 = 1;
    }
    else if(CH4>=60){
      Alart_message1 = "CH4 gas level is higher then 60ppm";
      if(BB1==1){
        bot.sendMessage(chat_id, "Non_Electrical fault occured: CH4 gas level is higher then 60ppm");
      }
      BB1 = 0;
      BB2 = 1;
    }
      ...
      if(BB2==1){
        bot.sendMessage(chat_id, "Normal Non_Electrical fault cleard");
      }
      else{}
      BB1 = 1;
      BB2 = 0;
 ...
 */
  ...
  if(Firebase.ready() && signupOK ) {
    if (Firebase.RTDB.setFloat(&fbdo, "Main/Electrical/VOLTAGE", voltage)){
      ...
      Serial.println(voltage2 + 0.1);  //0.1 is added to calibrate with PZEM1
    }
    else {
